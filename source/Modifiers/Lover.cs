...
namespace TownOfUsEdited.Modifiers
{
    public class Lover : Modifier
    {
        ...
        internal override void ModifierWin(LogicGameFlowNormal __instance)
        {
            if (CheckLoversWin())
            {
        ...
        public void Win()
        {
            if (CustomGameOptions.JesterWin == WinEndsGame.EndsGame && Role.AllRoles.Where(x => x.RoleType == RoleEnum.Jester).Any(x => ((Jester)x).VotedOut)) return;
