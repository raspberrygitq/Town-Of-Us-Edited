...
namespace TownOfUs.NeutralRoles.PhantomMod
{
    ...
    public class RepickPhantom
    {
        private static void Postfix(HudManager __instance)
        {
            ...
            var toChooseFrom = PlayerControl.AllPlayerControls.ToArray().Where(x => !x.Is(Faction.Crewmates) && !x.Is(Faction.Impostors) && !x.Is(ModifierEnum.Lover) && !x.Is(Faction.Madmates) && !x.Is(Faction.Coven) && x.Data.IsDead && !x.Data.Disconnected).ToList();
