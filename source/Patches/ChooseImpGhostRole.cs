...
namespace TownOfUsEdited.ChooseImpGhostRole
{
    ...
    public class PickImpRole
    {
        ...
        public static void PickGhostRole(PlayerControl exiled)
        {
            ...
            if (RpcHandling.Check(CustomGameOptions.WraithOn) && SetWraith.WillBeWraith == null && !ranlist && !GhostRoles.Contains("Wraith")) GhostRoles.Add("Wraith");
            ...
            toChooseFromImpo.RemoveAll(player => SetWraith.WillBeWraith == player);
            ...
            // Set the Wraith, if there is one enabled.
            if (chosenRole == "Wraith" && toChooseFromImpo.Count != 0)
            {
                var rand = UnityEngine.Random.RandomRangeInt(0, toChooseFromImpo.Count);
                var pc = toChooseFromImpo[rand];

                SetWraith.WillBeWraith = pc;

                Utils.Rpc(CustomRPC.SetWraith, pc.PlayerId);
            }
