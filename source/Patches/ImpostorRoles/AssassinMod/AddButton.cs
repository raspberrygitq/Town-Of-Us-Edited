    ...
    public class AddButtonAssassin2
    {
        ...
        private static Action Guess(Assassin role, PlayerVoteArea voteArea)
        {
            void Listener()
            {
                ...
                var toDie = ((playerRole.Name.Contains(currentGuess) && !playerRole.Player.Is(RoleEnum.Hunter)) || playerRole.Name == currentGuess) ? playerRole.Player : role.Player;
                ...
                if ((!toDie.Is(RoleEnum.Pestilence) || PlayerControl.LocalPlayer.Is(RoleEnum.Pestilence)) && !fortified)
                {
                    if (PlayerControl.LocalPlayer.Is(ModifierEnum.DoubleShot) && toDie == PlayerControl.LocalPlayer)
                    {
                        ...
                        if (modifier.LifeUsed == false)
                        {
                            ...
                            ShowHideButtonsAssassin.HideSingle(role, targetId, false, true);
                        }
                        else
                        {
                            AssassinKill.RpcMurderPlayer(toDie, PlayerControl.LocalPlayer);
                            role.RemainingKills--;
                            ShowHideButtonsAssassin.HideSingle(role, targetId, toDie == role.Player);
                            if (toDie.IsLover() && CustomGameOptions.BothLoversDie)
                            {
                                var lover = ((Lover)playerModifier).OtherLover.Player;
                                if (!lover.Is(RoleEnum.Pestilence)) ShowHideButtonsAssassin.HideSingle(role, lover.PlayerId, false);
                            }
                        }
                    }
                    else
