            ...
            foreach (var modifier in Modifier.AllModifiers.Where(x => x.ModifierType == ModifierEnum.Shy))
            {
                ...
                if (player.GetCustomOutfitType() == CustomPlayerOutfitType.Swooper)
                {
                    ...
                    if (PlayerControl.LocalPlayer.Data.IsImpostor() && player.Is(RoleEnum.Swooper) && !PlayerControl.LocalPlayer.IsHypnotised() ||
                        PlayerControl.LocalPlayer == player && player.Is(RoleEnum.Swooper) || player.Is(RoleEnum.Swooper) && PlayerControl.LocalPlayer.Data.IsDead ||
                        player.Is(RoleEnum.Chameleon) && PlayerControl.LocalPlayer.Data.IsDead || player.Is(RoleEnum.Chameleon) && PlayerControl.LocalPlayer == player) shy.Opacity = 0.1f;
