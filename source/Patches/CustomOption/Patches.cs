...
namespace TownOfUsEdited.CustomOption
{
    public static class Patches
    {
        ...
        private class SettingsUpdate
        {
            ...
            public static void CreateSettings(GameSettingMenu __instance, int target, string name, string text, GameObject settingsButton, MultiMenu menu)
            {
                ...
                if (target < 10)
                {
                    ...
                    if (target == 3)
                    {
                        ImportText = SpawnExternalButton(__instance, tabOptions, ref num, "Load Custom Settings", () => Coroutines.Start(TabPatches.ChangeTab(__instance, 9)));
                        ExportText = SpawnExternalButton(__instance, tabOptions, ref num, "Save Custom Settings", () => Coroutines.Start(TabPatches.ChangeTab(__instance, 10)));
                    }
        ...
        class SetTabPane
        {
            ...
            {
                if ((int)__instance.currentTab < 7)
                {
