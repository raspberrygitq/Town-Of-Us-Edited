    ...
    public static class RpcHandling
    {
        ...
        private static void GenEachRole(List<NetworkedPlayerInfo> infected, int impsCount)
        {
            ...
            if (CustomGameOptions.GameMode == GameMode.Classic)
            {
                ...
                while (uppedPlayers.Count > 0 && CustomGameOptions.GameMode == GameMode.Classic && CustomGameOptions.AllowUp)
                {
                    ...
                    foreach (var pc in players)
                    {
                        ...
                        else if (roleName == "Juggernaut" && CustomGameOptions.HiddenRoles)
                        {
                            Role.GenRole<Role>(typeof(Juggernaut), pc);
                            uppedPlayers.Remove(pc);
                            crewmates.Remove(pc);
                            killing--;
                        }
            ...
            var canHaveModifier = PlayerControl.AllPlayerControls.ToArray()
                .Where(player => !player.Is(ModifierEnum.Disperser) && !player.Is(ModifierEnum.DoubleShot) && !player.Is(ModifierEnum.Underdog) && !player.Is(ModifierEnum.Lucky))
                .ToList();
        ...
        private static void GenEachRoleKilling(List<NetworkedPlayerInfo> infected, int impsCount)
        {
            var crewmates = Utils.GetPlayers(infected);
            var impostors = new List<PlayerControl>();
            ...
            NeutralKillingRoles.Add((typeof(Doppelganger), 10, true));
            NeutralKillingRoles.Add((typeof(Mutant), 10, true));
            NeutralKillingRoles.Add((typeof(Maul), 10, true));
            ...
            public static void Postfix([HarmonyArgument(0)] byte callId, [HarmonyArgument(1)] MessageReader reader)
            {
                ...
                switch ((CustomRPC) callId)
                {
                    ...
                    case CustomRPC.SetManipulate:
                        ...
                        manipulatorRole.IsManipulating = true;
                        break;
                    case CustomRPC.SetManipulateOff:
                        ...
                        manipulatorRole1.ManipulatedPlayer = null;
                        manipulatorRole1.IsManipulating = false;
                        break;
            ...
            public static void Postfix()
            {
                ...
                || CustomGameOptions.GameMode == GameMode.Cultist)
                {
                    ...
                    if (CustomGameOptions.VultureOn > 0)
                        NeutralEvilRoles.Add((typeof(Vulture), CustomGameOptions.VultureOn, false));

                    if (CustomGameOptions.TrollOn > 0)
                        NeutralEvilRoles.Add((typeof(Troll), CustomGameOptions.TrollOn, false));
                ...
                if (CustomGameOptions.GameMode == GameMode.KillingOnly) GenEachRoleKilling(infected.ToList(), __result);
                else if (CustomGameOptions.GameMode == GameMode.Werewolf) GenEachRoleWerewolf(infected.ToList(), __result);
